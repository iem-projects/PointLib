...
object Evolutions4 extends App {
  val NUM         = 200
  val SEED        = 12311L  // seed of the random number generator
  val START       = 6       // start index in the pitch sequence to begin wih
...
//  val snippets  = /* staticChords(5).last :: */ improvSnippets  // staticChords(5).head
  val snippets  = improvSnippets.reverse // .last :: Nil  // staticChords(5).head
  var notesIn0  = Vector.empty[OffsetNote]
  snippets.foreach { i =>
    val sn  = loadSnippet(i).notes
    val off = notesIn0.lastOption.fold(0.0)(n => n.stop + n.duration)
    val n2  = sn.map(n => n.copy(offset = n.offset + off))
    notesIn0 ++= n2
  }

//  val disk = loadDisklavier(DISK_IDX)
//  notesIn0 ++= disk.notes
  ...
  var produced  = 0
  var tpe       = 'm' // 'h' // alternates between 'h' and 'm'
  ...
  while (produced < NUM) {
    ...
    val off  = res.lastOption.fold(0.0)(n => n.stop + n.duration)
  ...
  //     s"${if (GRAINS) "G" else ""}${if (VELO) "V" else ""}${if (ENTRY) "E" else ""}.mid"))
    ...
    sqOut.writeFile(outPath /
      (s"Snippet${snippets.mkString("_")}Neu_Resynth_${START}_${SEED}${if (INNER) "I" else ""}" +
        s"${if (GRAINS) "G" else ""}${if (VELO) "V" else ""}${if (ENTRY) "E" else ""}.mid"))

//  sqOut.writeFile(outPath /
//    (s"Disklavier${DISK_IDX}Neu_Resynth_${START}_${SEED}${if (INNER) "I" else ""}" +
//     s"${if (GRAINS) "G" else ""}${if (VELO) "V" else ""}${if (ENTRY) "E" else ""}.mid"))
