...
object PDFSupport {
  def addMenu[A <: JComponent](f: JFrame, views: Seq[A], prepare: A => Unit = (_: A) => (), usePrefSize: Boolean = true) {
    ...
    val miExportPDF = new JMenuItem(new AbstractAction("Export as PDF...") { action =>
      ...
      def actionPerformed(e: ActionEvent) {
        val viewO: Option[A] = views.toList match {
          ...
          case _ =>
            ...
            val icons: IndexedSeq[Icon] = viewsi.zipWithIndex.map({
              case (view, idx) =>
                new Icon {
                  def getIconWidth  = w
                  def getIconHeight = h
            ...
            list.setCellRenderer(new DefaultListCellRenderer {
              override def getListCellRendererComponent(l: JList, value: AnyRef, idx: Int, isSelected: Boolean,
        ...
        viewO foreach { view =>
          ...
          prepare(view)
          createPDF(new File(dir, fileExt), view, usePrefSize)
        }
      }
    })
